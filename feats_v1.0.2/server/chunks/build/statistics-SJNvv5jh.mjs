...
import { ssrRenderAttrs, ssrInterpolate, ssrRenderClass, ssrRenderComponent, ssrRenderList, ssrRenderStyle } from 'vue/server-renderer';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const yearTotalsChartData = computed(() => {
      const data = stats.value.yearTotals.filter((item) => item.year !== "Grand Total");
      return {
        ...
        datasets: [
          {
            ...
            data: sorted.map((item) => item.longest),
            borderColor: "#007a76",
    ...
    const perBikeChartData = computed(() => {
      ...
      return {
        labels: data.map((item) => item.bike),
        datasets: [
          {
            label: "Distance (km)",
            data: data.map((item) => item.distance),
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<section${ssrRenderAttrs(_attrs)}><h1 class="text-3xl font-semibold mb-6">Statistics</h1><div class="fun-fact mb-8 p-4"><p class="text-lg font-semibold text-center"> Great! You cycled <span class="circumferences font-bold">${ssrInterpolate(earthCircumferences.value)}</span> times around the earth. Keep on pedaling! </p></div><div class="fts-tabs mb-6 flex gap-2 border-b color:light:border-gray-300 dark:border-gray-700"><button class="${ssrRenderClass([activeTab.value === "tables" ? "is-active" : "", "fts-tab px-4 py-2 -mb-px border-b-2"])}"> Tables </button><button class="${ssrRenderClass([activeTab.value === "charts" ? "is-active" : "", "fts-tab px-4 py-2 -mb-px border-b-2"])}"> Charts </button></div>`);
      if (activeTab.value === "tables") {
        _push(`<div><div class="grid gap-15 lg:grid-cols-3"><div><h2 class="text-xl font-semibold mb-3">Year Totals</h2><div class="table-statistics bg-white border">`);
        ...
        _push(`<div class="w-full"><div class="space-y-8 w-full"><div class="bg-white dark:bg-gray-800 p-4 rounded border w-full"><h2 class="text-xl font-semibold mb-4">Year Totals</h2><div style="${ssrRenderStyle({ "height": "500px", "width": "100%" })}">`);
        _push(ssrRenderComponent(unref(Bar), {
          data: yearTotalsChartData.value,
          options: yearTotalsChartOptions
        }, null, _parent));
        _push(`</div></div><div class="bg-white dark:bg-gray-800 p-4 rounded border w-full"><h2 class="text-xl font-semibold mb-4">Longest Ride per Year</h2><div style="${ssrRenderStyle({ "height": "400px", "width": "100%" })}">`);
        _push(ssrRenderComponent(unref(Line), {
          data: longestPerYearChartData.value,
          options: longestPerYearChartOptions
        }, null, _parent));
        _push(`</div></div><div class="bg-white dark:bg-gray-800 p-4 rounded border w-full"><h2 class="text-xl font-semibold mb-4">Distances and Rides per Bike in ${ssrInterpolate(currentYear.value)}</h2><div style="${ssrRenderStyle({ "height": "400px", "width": "100%" })}">`);
        _push(ssrRenderComponent(unref(Bar), {
          data: perBikeChartData.value,
          options: perBikeChartOptions
        }, null, _parent));
...
//# sourceMappingURL=statistics-SJNvv5jh.mjs.map
