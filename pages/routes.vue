<template>
  <section>
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center space-x-2">
        ...
        <div class="relative">
          ...
          <div v-if="showCols" class="absolute right-0 mt-2 bg-white border p-2 shadow">
    ...
    <div class="bg-white border overflow-x-auto relative">
      <Table :data="rows" :columns="tableColumns" :state="tableState" :onStateChange="onStateChange" :onSelect="(e,row)=>handleTableSelect(e,row)" />
...
const visibleColumns = ref(JSON.parse(localStorage.getItem('routes_visible_columns') || 'null') || allColumns.map(c=>c.key))
...
const tableState = ref({ globalFilter: '', sorting: [], pagination: { pageIndex: 0, pageSize: 10 }, columnVisibility: JSON.parse(localStorage.getItem('routes_column_visibility') || 'null') || Object.fromEntries(allColumns.map(c=>[c.key, true])) })
...
const tableColumns = [
  ...
  { accessorKey: 'link', headerLabel: 'Link', cell: ({ getValue, row }) => {
      const value = getValue()
      if (row.original.reference) return h('a', { href: row.original.reference, target: '_blank', class: 'text-blue-600 hover:underline' }, value || 'Link')
      return value || ''
    }
  },
...
function nextPage(){
  ...
  if ((pageIdx+1)*ps < total) tableState.value.pagination.pageIndex++
}
